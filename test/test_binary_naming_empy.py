...
def test_conditional_python_logic_in_empy():
    template = "@[if package.startswith('gz')]gz-@(distro)-@(package_suffix)@[else]ros-@(distro)-@(package_hyphens)@[end if]"
...
def test_distribution_file_empy_templating_and_reverse_lookup():
    dist_data = {
        'type': 'distribution',
        'version': 3,
